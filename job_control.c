    ...
    while (*args) {
        ...
        if (is_in || is_out) {
            ...
            if (*args){
                ...
                char **aux = args + 1;
                while (*aux) {//Removes the redirection symbol from the args original
                   *(aux-2) = *aux;
                   aux++;
                }
                *(aux-2) = NULL;
...
{
	/* Suspended process */
	if (WIFSTOPPED (status))//Child was stopped by signal
	{
		*info=WSTOPSIG(status);
		return(SUSPENDED);
	}
	/* Continued process */
    else if (WIFCONTINUED(status))//Resumed after being stopped
    { 
        *info=0; 
        return(CONTINUED);
    }
	...
	else if (WIFEXITED (status))//Exited normally
	{
		*info=WEXITSTATUS(status);
		return(EXITED);
	}
	/* Should never get here*/// As a defensive barrier if something go wrong
	return -1;
}
